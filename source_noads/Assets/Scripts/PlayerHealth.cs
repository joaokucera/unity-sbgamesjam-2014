...
class PlayerHealth : MonoBehaviour
{
    ...
    private const float LoseMultiplierTime = 10f;
    private float timeToLoseMultiplier;

    private float BonusTimer = 10f;
    ...
    void Update()
    {
        ...
        if (keepIncrease)
        {
            ...
            if (currentLife == StartLife)
            {
                ...
                if (timeToMultiplierFirstBonus >= BonusTimer)
                {
                    ChangeMultiplier(1);
    ...
    void OnTriggerEnter2D(Collider2D collider)
    {
        if (collider.tag.Contains("Obstacle"))
        {
            ...
            if (TryChangeBoard(obstacleScript))
            {
    ...
    private bool TryChangeBoard(GenericEnemy genericEnemy)
    {
        if (this.obstacleType == genericEnemy.ObstacleType)
        {
        ...
        else
        {
            ...
            //{
            //    firstObstacleHitScript = genericEnemy;
            //}
            ...
            this.obstacleType = genericEnemy.ObstacleType;
    ...
    private void GetHurt()
    {
        ...
        if (currentLife <= 0)
        {
            ...
            Animator scoreAnimator = GameObject.Find("Score").GetComponent<Animator>();
